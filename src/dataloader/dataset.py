...
class TabularDataset(Dataset):
    ...
    def merge(self, other: 'Dataset'):
        ...
        # Special rule: ADReSS_with_PITT does not have a sample for S200
        # We allow this explicitly here
        if set(other.sample_names).difference(set(self.sample_names)) == {'S200'}:
            print(f"\nWARNING: removing sample S200 from dataset {str(other)}. This is expected when combining dataset ADReSS_with_PITT with ADReSS", end="\n\n")
            index_of_S200 = list(other.sample_names).index('S200')
            indices_without_S200 = [idx for idx in range(len(other.sample_names)) if idx != index_of_S200]
            # replace other with same dataset without S200
            other = other.subset_from_indices(indices_without_S200)
        elif set(self.sample_names).difference(set(other.sample_names)) == {'S200'}:
            raise NotImplementedError("Dataset merging: S200 exists in self, but not in other. This is allowed the other way round, but nees implementation here. Consider switching the order around")
